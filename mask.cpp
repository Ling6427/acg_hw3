...
int main(int argc, char **argv)
{
    ...
    // load images
    int mw, mh, mn;
    unsigned char *mdata = stbi_load("einsteinMask.bmp", &mw, &mh, &mn, 0);
    cout<<"mw: "<<mw<<", mh: "<<mh<<", n: "<<mn<<endl;
    ...
    for(int j=0; j<ih; j++) {
        for(int i=0; i<iw; i++) {
            ...
            if(comparecolor(mdata, index, 255, 255, 255)) {
